...
export class AuthService {
  ...
  async validateUserByJwt(payload: any) {
    try {
      const p = await this.JwtService.verifyAsync(payload, {
        secret: process.env.ACCESS_TOKEN_SECRET,
      });
      console.log(p);
      return p;
  ...
  async SendResetPassword(dto: SendResetPasswordDTO) {
    ...
    await this.mailService.sendPassReset(
      dto.email,
      `${process.env.HOST_URL}/reset/${token}`,
      user.name,
    );
  ...
  async setNewPassword(dto: SetNewPasswordDTO) {
    ...
    const createNewPassword = await this.userService.updateUserPassword(
      user.id,
      dto.password,
    );
